...
export function useSales(filters: {
  ...
  const salesQuery = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      if (filters.deleted !== undefined) qp.set("deleted", String(filters.deleted));
      if (filters.search) qp.set("search", String(filters.search));
      ...
      const res = await fetch(`/api/sales/load?${qp.toString()}`, { credentials: "include" });
